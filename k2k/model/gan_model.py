...
def init_net(net, init_type="normal", gain=0.02):
    ...
    Args:
        net (model): 要使用的网络
        init_type (str, optional): 参数初始化的. Defaults to "normal".
        gain (float, optional): 暂且未知. Defaults to 0.02.
    """

    def init_func(m):
        classname = m.__class__.__name__
        # hasattr判断类是否包含该属性
        # init是torch提供的函数，提供不同的方法初始化权重
...
def init_NLayerDiscriminator(input_nc, ndf=64, n_layers=3, norm_layer=nn.BatchNorm2d, use_sigmoid=False):
    ...
    Args:
        input_nc (int): 输入的size
        ndf (int, optional): 第一层的默认输出. Defaults to 64.
        n_layers (int, optional): 层数. Defaults to 3.
        norm_layer (每层之后使用的norm函数, optional): 略. Defaults to nn.BatchNorm2d.
        use_sigmoid (bool, optional): 是否使用sigmoid. Defaults to False.
    ...
    for n in range(1, n_layers):
        nf_mult_prev = nf_mult  # 输入倍数
        nf_mult = min(2 ** n, 8)  # 输出倍数，小于8
        sequence += [
    ...
    model = nn.Sequential(*sequence)  # 星号代表独立参数，传入seq类
    return model
...
class GANModel(ModelBase):
    def __init__(self, args):
        ...
        if args.netD_type == "basic":
            self.model = init_NLayerDiscriminator(
